...
public class RotationalConverterTests
{
	...
	{
		// Arrange
		var tolerance = 0.001; // Tolerância para diferenças devido a erros de ponto flutuante
	...
	[Theory]
	[InlineData("G1 X0 Y0 Z0", "G1 X0 Y0 Z0", 360)] // Sem alteração
	[InlineData("G1 X10 Y10 Z0", "G1 X45 Y14.14 Z0", 360)] // Conversão com fator 360
	[InlineData("G1 X10 Y10 Z0", "G1 X12.5 Y14.14 Z0", 100)] // Conversão com fator 100
	...
	public void TestConvertToRotational_SingleLine(string inputLine, string expectedLine, int thetaConversionFactor)
	{
		...
		var command = GCodeFileHandler.ParseGCodeLine(inputLine);
		if (command != null)
		{
		...
		// Extrai a primeira linha do resultado
		var resultLine = result.Commands.Count > 0 ? result.Commands[0].ToString() : string.Empty;

		// Remove zeros desnecessários (opcional, dependendo da necessidade)
		var trimmedResult = ZeroTrimmer.TrimZeros(new List<string> { resultLine });
